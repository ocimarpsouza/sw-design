...
public class AgendaApp {
	...
	public static void main(String[] args) {
		boolean sair = false;
		while(!sair){
			int opcao = aprensetarMenuPrincipal();
			switch (opcao) {
			case 1: inserirContato(); break;
			case 2: buscarContato(); break;
			case 3: sair =true; break;				
			default: out.println("ERRO: opção inválida!");break;
			}
		}
		out.println("\nFim do programa");
	}
	
	private static int aprensetarMenuPrincipal(){
	...
	private static void inserirContato(){
		...
		if(dao.existe(c)){
			out.println("Este contnato já está cadastrada!");
		}else{
			//contatos.add(c);
			dao.inserir(c);
